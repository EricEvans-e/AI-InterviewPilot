...
当前系统已经具备题库面试的基础链路：考生选择院校、专业和面试类型后，后端从 `question` 表中按 `collegeId`、`majorId`、`questionType` 随机抽取 `approved` 状态题目，并写入 `interview_session_question`，随后面试页直接按题号读取这些题目。
...
第一阶段可以用 SQL 精确查询和文本拼接形成素材包，后续再升级为向量检索 RAG。
...
- 从大量题库和考纲中找最相关材料。
- 避免 AI 凭空出题。
- 让 AI 能根据相近专业、相近能力点进行举一反三。
- 支持输出来源依据，方便老师审核。

第一阶段不依赖 RAG，避免过早增加复杂度。
...
```http
GET /api/ip/v1/questions/coverage?collegeId=1&majorId=2&interviewMode=专业认知
```
...
最适合当前项目的路径不是一开始就做复杂 RAG，而是先把题库建设、导入、扩题、审核和快速抽题闭环跑通。

题库负责稳定性，离线扩题负责丰富度，审核负责质量，临时生成负责冷门场景兜底。这样既能提升学生体验，也能控制 token 成本，并为后续 RAG 知识库留下清晰接口。
