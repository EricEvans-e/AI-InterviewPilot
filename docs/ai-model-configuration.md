...
InterviewSessionFacade → InterviewAiInvoker.doChat()
    ...
    └─ aiProvider=anthropic   → AnthropicPromptBuilder + AnthropicChatHandler.callSync()
    → agent_properties 表
```
...
InterviewAiInvoker.doChat()
    ...
    └─ "anthropic" → AnthropicPromptBuilder.build() → AnthropicChatHandler.callSync()
```

`AnthropicPromptBuilder` 将 XingChen 的结构化 workflow 参数（`AGENT_USER_INPUT`、`question`、`resume_context` 等）转换为纯文本 prompt，自动适配评分、追问等场景。

注意事项：
- Anthropic 没有文件上传 API，出题场景跳过简历上传，直接用纯文本 prompt
- 神态分析场景在 Anthropic 模式下跳过图片上传，使用文本分析
