...
INSERT INTO ai_properties (
    ...
    max_tokens, temperature, system_prompt, is_enabled, del_flag, create_time, update_time
) VALUES (
...
```sql
INSERT INTO ai_properties (ai_name, ai_type, api_key, api_url, model_name, max_tokens, temperature, is_enabled, del_flag, create_time, update_time)
VALUES ('DeepSeek V3', 'deepseek', 'sk-xxx', 'https://api.deepseek.com', 'deepseek-chat', 8192, 0.7, 1, 0, NOW(), NOW());
```
...
```sql
INSERT INTO ai_properties (ai_name, ai_type, api_key, api_url, model_name, max_tokens, temperature, is_enabled, del_flag, create_time, update_time)
VALUES ('豆包 Pro', 'doubao', 'ak-xxx', 'https://ark.cn-beijing.volces.com/api/v3', 'doubao-pro-32k', 8192, 0.7, 1, 0, NOW(), NOW());
```
...
```sql
INSERT INTO ai_properties (ai_name, ai_type, api_key, api_url, model_name, max_tokens, temperature, is_enabled, del_flag, create_time, update_time)
VALUES ('星火 4.0', 'spark', 'ak-xxx', 'https://spark-api-open.xf-yun.com/v1', 'generalv3.5', 8192, 0.7, 1, 0, NOW(), NOW());
```
...
3. 选择 AI 类型为 `mimo`
4. 填写生成条件（院校、专业、题型等）
5. 系统通过 `MimoChatHandler.callSync()` 同步调用 Mimo 生成题目
...
`ai_properties` 表中同一 `ai_type` 只有第一条启用记录会被使用。如需切换，禁用旧记录或调整 `is_enabled` 状态。
