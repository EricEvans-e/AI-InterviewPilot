...
- 接口：`POST /api/ip/v1/agents/sessions`
...
- 默认 Mimo provider 通过 `UniversalAiChatHandler` 或 `AnthropicChatHandler` 流式写入 `SseEmitter`；只有 legacy `xingchen` provider 会调用 `XunfeiWorkflowClient.chat(...)`。
...
- 接口：`PUT /api/ip/v1/agents/conversations/{sessionId}/end`
