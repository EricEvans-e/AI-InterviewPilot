...
- 先判断对象是什么，再判断它属于哪个域。
- 面试会话、出题、评分、追问、恢复、简历预览、神态分析，都归 `interview-pilot-interview-domain`。
- 通用 Agent 聊天、会话历史、文件上传、Agent 属性管理，都归 `interview-pilot-agent-domain`。
- 实时转写、WebSocket 推送、长文本 TTS，都归 `interview-pilot-media-domain`。
- 运行时限流、守护、singleflight、线程池、配置语义，都归 `interview-pilot-ai-runtime`。
- 登录、当前用户、权限、WebSocket 鉴权，都归 `interview-pilot-auth-user`。
- 需求同时改接口、DTO、工作流、配置、缓存时，先用 `interview-pilot-change-playbook`。
- 现象不清楚、怀疑卡死、重试、重复、恢复失败时，先用 `interview-pilot-debug-playbook`。
