...
- **实时语音转写（ASR）**：基于 WebSocket + Mimo ASR 实现端到端实时语音识别，支持服务端音频缓冲、PCM 转 WAV 和最终文本推送。
- **长文本语音合成（TTS）**：集成 Mimo TTS，支持创建合成任务、查询状态、同步等待三种模式。
...
4. 将实时 ASR/TTS 链路迁移到 Mimo 模型，前端通过 `/api/ip/v1/mimo/audio-to-text/{userId}` 推送音频，后端统一完成 PCM 转 WAV、ASR 调用、TTS 合成和旧路径兼容。
