...
**InterviewPilot平台** - 基于大语言模型的简历分析、模拟面试服务
...
- **Mimo 统一接入**：基于 `AiChatHandlerFactory` 策略路由，`UniversalAiChatHandler` 走 Mimo OpenAI 兼容协议，`AnthropicChatHandler` 走 Mimo Anthropic 兼容协议，运行时可切换。
...
- **实时语音转写（ASR）**：基于 WebSocket + Mimo ASR 接收浏览器 PCM 音频，服务端缓冲后封装为 WAV，并在停止转写时推送最终文本。
- **长文本语音合成（TTS）**：集成 Mimo TTS，同步合成音频并以兼容 task 形态返回结果。
...
4. 将 ASR/TTS 链路迁移到 Mimo 模型，前端通过 `/api/ip/v1/mimo/audio-to-text/{userId}` 推送音频，后端在停止转写后完成 PCM 转 WAV 与 ASR 调用；TTS 同步合成音频并保留旧路径兼容。
