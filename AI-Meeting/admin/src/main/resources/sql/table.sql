...
(
    ...
    source_platform varchar(32) default 'mimo-local'      not null comment '来源平台',
...
(
    ...
    api_flow_id varchar(512) null comment '工作流id或API基础地址',
    ai_provider varchar(50)  default 'openai' null comment 'AI 提供商: openai, anthropic, legacy xingchen',
    scene_code  varchar(100) default null comment '业务场景绑定编码',
    is_active   tinyint(1)   default 0 null comment '是否为该场景当前激活agent',
...
(
    ...
    ai_type                varchar(64)                not null comment 'AI类型：openai、anthropic',
    ...
    enable_thinking        tinyint(1)    default 0    null comment '是否开启思考模式（Mimo Pro/Anthropic兼容） 0：关闭 1：开启',
    thinking_budget_tokens int                        null comment '思考模式预算Token数（Mimo Pro/Anthropic兼容）',
    create_time            datetime                   null comment '创建时间',
    update_time            datetime                   null comment '修改时间',
    del_flag               tinyint(1)    default 0    null comment '删除标识 0：未删除 1：已删除',
