package com.interviewpilot.toolkit.iflytek;
...
public class XunfeiWorkflowClient {
    ...
    public void chat(String input,
        ...
        log.debug("Dispatching Xunfei workflow chat request, chatId={}, stream={}, hasFile={}, extraParameterCount={}",
                chatId,
                stream,
        ...
        log.info("Xunfei workflow chat response code={}", responseCode);
        if (responseCode != HttpsURLConnection.HTTP_OK) {
            throw new IOException("Xunfei workflow chat request failed, status=" + responseCode
    ...
    public String uploadFile(MultipartFile file, String apiKey, String apiSecret) throws Exception {
        ...
        int responseCode = conn.getResponseCode();
        log.info("Xunfei workflow upload response code={}", responseCode);
        if (responseCode == HttpsURLConnection.HTTP_OK) {
            String responseBody = readResponseBody(conn, false);
            log.info("Xunfei workflow upload response={}", responseBody);
            return parseFileUrlFromResponse(responseBody);
        }

        String errorBody = readResponseBody(conn, true);
        log.error("Xunfei workflow upload failed: {}", errorBody);
    ...
    private void handleStreamingResponse(HttpsURLConnection conn,
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                log.debug("Received Xunfei workflow SSE chunk, chatId={}, payloadLength={}",
    ...
    private String parseFileUrlFromResponse(String responseBody) {
        try {
            ...
            log.info("Parsed Xunfei workflow file URL={}", url);
            return url;
        } catch (Exception ex) {
            log.error("Failed to parse Xunfei workflow file URL, response={}", responseBody, ex);
