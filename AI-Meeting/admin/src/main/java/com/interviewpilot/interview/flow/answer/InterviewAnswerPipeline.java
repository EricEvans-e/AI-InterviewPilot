package com.interviewpilot.interview.flow.answer;
...
import com.interviewpilot.interview.application.runtime.InterviewSessionRuntimeView;
import com.interviewpilot.interview.application.rule.InterviewFollowUpRuleContext;
import com.interviewpilot.interview.application.rule.InterviewFollowUpRuleDecision;
import com.interviewpilot.interview.application.rule.InterviewFollowUpRuleService;
...
public class InterviewAnswerPipeline {
    ...
    private boolean stepLoadCurrentQuestion(InterviewAnswerPipelineContext ctx) {
        ...
        // 检测题库模式并加载题目元数据（评分规则、预设追问）
        loadQuestionBankContext(ctx);
    ...
    private boolean stepEvaluateAndScore(InterviewAnswerPipelineContext ctx) {
        ...
        // 题库模式下，将评分规则附加到题目内容中，引导 AI 按规则评分。
        String questionWithContext = ctx.currentQuestion;
        if (ctx.questionBankMode && StrUtil.isNotBlank(ctx.questionBankScoringRule)) {
            questionWithContext = ctx.currentQuestion + "\n\n【评分规则】\n" + ctx.questionBankScoringRule;
        }
    ...
    /**
     * 检测题库模式并加载当前题目的评分规则和预设追问。
     */
    private void loadQuestionBankContext(InterviewAnswerPipelineContext ctx) {
        try {
            InterviewSession session = interviewSessionService.getBySessionId(ctx.sessionId);
            if (session == null || !"questionBank".equals(session.getSessionMode())) {
                ctx.questionBankMode = false;
                return;
            }
            ...
            InterviewSessionQuestionDO link = sessionQuestionMapper.selectOne(
                    Wrappers.lambdaQuery(InterviewSessionQuestionDO.class)
                            .eq(InterviewSessionQuestionDO::getSessionId, ctx.sessionId)
                            .eq(InterviewSessionQuestionDO::getSeqIndex, seqIndex)
            );
            if (link == null) {
                return;
            }

            QuestionDO questionDO = questionBankService.getById(link.getQuestionId());
            if (questionDO == null) {
                return;
            }
