...
describe("request utilities", () => {
  it("requires auth token for protected business endpoints", () => {
    expect(requiresAuthTokenForRequest("/ip/v1/interview/sessions")).toBe(
      true,
    );
    expect(requiresAuthTokenForRequest("/ip/v1/users/login")).toBe(false);
  });
  ...
  it("throws unauthorized before request when protected endpoint has no token", () => {
    ...
    expect(() =>
      assertRequestAuthorized("/ip/v1/interview/sessions"),
    ).toThrow(AppError);
    expect(() =>
      assertRequestAuthorized("/ip/v1/interview/sessions"),
  ...
  it("allows protected endpoint when token exists", () => {
    ...
    expect(assertRequestAuthorized("/ip/v1/interview/sessions")).toBe(
      "token-value",
    );
  });
