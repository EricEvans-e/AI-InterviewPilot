...
export type CreateMimoTtsTaskParams = {
  text: string;
  vcn?: string;
  language?: string;
  speed?: number;
...
const toTaskError = (task: MimoTtsTaskResult, fallback: string) =>
  new Error(task.message || fallback);

export const mimoTtsService = {
  async createTask(
    params: CreateMimoTtsTaskParams,
    options?: RequestOptions,
  ) {
    const response = await service.post<
      MimoTtsTaskResult,
      CreateMimoTtsTaskParams
    >("/ip/v1/mimo/tts/tasks", params, {
  ...
  async queryTask(taskId: string, options?: RequestOptions) {
    const response = await service.get<MimoTtsTaskResult>(
      `/ip/v1/mimo/tts/tasks/${encodeURIComponent(taskId)}`,
      {
        signal: options?.signal,
      },
    );
  ...
  async synthesize(
    ...
    const response = await service.post<
      MimoTtsTaskResult,
      CreateMimoTtsTaskParams
    >("/ip/v1/mimo/tts/synthesize", params, {
      signal: options?.signal,
    });
