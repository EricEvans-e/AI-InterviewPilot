...
const normalizeInterviewRadarChart = (
  ...
  return {
    ...
    totalScore:
      ...
      payload.totalScore,
    contentScore:
      toNumberValue(pickFirst(source, ["contentScore", "content_score"])) ??
      payload.contentScore,
    logicScore:
      toNumberValue(pickFirst(source, ["logicScore", "logic_score"])) ??
      payload.logicScore,
    professionalScore:
      toNumberValue(pickFirst(source, ["professionalScore", "professional_score"])) ??
      payload.professionalScore,
    expressionScore:
      toNumberValue(pickFirst(source, ["expressionScore", "expression_score"])) ??
      payload.expressionScore,
    adaptabilityScore:
      toNumberValue(pickFirst(source, ["adaptabilityScore", "adaptability_score"])) ??
      payload.adaptabilityScore,
    timeControlScore:
      toNumberValue(pickFirst(source, ["timeControlScore", "time_control_score"])) ??
      payload.timeControlScore,
    etiquetteScore:
      toNumberValue(pickFirst(source, ["etiquetteScore", "etiquette_score"])) ??
      payload.etiquetteScore,
    compositeScore:
      toNumberValue(pickFirst(source, ["compositeScore", "composite_score"])) ??
...
const buildResumePreviewPath = (sessionId: string) =>
  `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/resume/preview`;
...
export const interviewService = {
  createInterviewSession: async () => {
    return service.post<CreateInterviewSessionResult, Record<string, never>>(
      "/ip/v1/interview/sessions",
      {},
    );
  },
  createSessionFromBank: async (params: CreateSessionFromBankParams) => {
    return service.post<CreateInterviewSessionResult, CreateSessionFromBankParams>(
  ...
  restoreInterviewSession: async (sessionId: string) => {
    const response = await service.get<InterviewSessionRestoreResult>(
      `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/restore`,
    );
  ...
  extractInterviewQuestions: async (
    ...
    const response = await service.post<
      ...
      FormData
    >(
      `/ip/v1/interview/sessions/${encodeURIComponent(params.sessionId)}/interview-questions`,
      formData,
      {
        timeout: INTERVIEW_LONG_TIMEOUT_MS,
        headers: {
  ...
  answerInterviewQuestion: async (params: AnswerInterviewQuestionParams) => {
    ...
    if (!params.audioFile) {
      ...
      try {
        ...
        >(
          `/ip/v1/interview/sessions/${encodeURIComponent(params.sessionId)}/interview/answer-json`,
          payload,
          {
            timeout: INTERVIEW_LONG_TIMEOUT_MS,
    ...
    const response = await service.post<
      ...
      FormData
    >(
      `/ip/v1/interview/sessions/${encodeURIComponent(params.sessionId)}/interview/answer`,
      formData,
      {
        timeout: INTERVIEW_LONG_TIMEOUT_MS,
        requestPolicy: answerRequestPolicy,
  ...
  getNextQuestion: async (sessionId: string) => {
    const response = await service.get<AnswerInterviewQuestionResult>(
      `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/next-question`,
    );
    return normalizeInterviewAnswer(response);
  },
  getCurrentQuestion: async (sessionId: string) => {
    try {
      const response = await service.get<AnswerInterviewQuestionResult>(
        `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/current-question`,
      );
      return normalizeInterviewAnswer(response);
    } catch (error) {
      if (shouldFallbackToLegacyPath(error)) {
  ...
  evaluateInterviewDemeanor: async (
    ...
    return service.post<string, FormData>(
      `/ip/v1/interview/sessions/${encodeURIComponent(params.sessionId)}/demeanor-evaluation`,
      formData,
      {
        timeout: INTERVIEW_LONG_TIMEOUT_MS,
        headers: {
  ...
  finishInterviewSession: async (sessionId: string) => {
    return service.put<void, Record<string, never>>(
      `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/finish`,
      {},
    );
  },
  ...
  pageInterviewRecords: async (params: PageInterviewRecordsParams) => {
    return getWithPathFallbackAndConfig<InterviewRecordsPageResult>(
      "/ip/v1/interview/interview/records",
      "/ip/v1/interview/records",
      {
        params,
      },
    );
  ...
  getInterviewRadarChart: async (sessionId: string) => {
    const response = await service.get<InterviewRadarChartResult>(
      `/ip/v1/interview/sessions/${encodeURIComponent(sessionId)}/radar-chart`,
    );
