...
export const authService = {
  ...
  register: (data: UserRegisterReqDTO) => {
    return service.post<ResultVoid>("/ip/v1/users/register", data);
  },

  checkLogin: async () => {
    const payload = await service.get<AuthPayloadDTO>(
      "/ip/v1/users/check-login",
    );
    const token = extractTokenFromAuthPayload(payload);
    if (token) {
      setAuthToken(token);
  ...
  logout: async () => {
    try {
      return await service.post<ResultVoid>("/ip/v1/users/logout");
    } finally {
      clearAuthToken();
    }
  },
  ...
  getUser: (username: string) => {
    return service.get<UserRespDTO>(`/ip/v1/users/${username}`);
  },

  getUserActual: (username: string) => {
    return service.get<UserActualRespDTO>(
      `/ip/v1/users/actual/${username}`,
    );
  },
  ...
  sendSmsCode: async (
    phone: string,
    bizType: string = "login",
  ): Promise<void> => {
    await service.post("/ip/v1/users/send-sms-code", null, {
      params: { phone, bizType },
    });
  },

  phoneLogin: async (phone: string, code: string): Promise<UserRespDTO> => {
    const payload = await service.post<AuthPayloadDTO>(
      "/ip/v1/users/phone-login",
      { phone, code },
    );
    const token = extractTokenFromAuthPayload(payload);
    if (token) {
    ...
    const user = extractUserFromAuthPayload(payload);
    if (!user) {
      throw new Error("Phone login succeeded but user info is missing");
    }
    if (!token) {
      throw new Error("Phone login succeeded but token is missing");
    }
    return user;
  },
};
