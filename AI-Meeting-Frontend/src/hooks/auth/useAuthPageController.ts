...
import { clearError, loginUser, selectUserRole } from "@/store/slices/userSlice";
...
export function useAuthPageController() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      const inAppRedirect = normalizeInAppRedirect(redirectState?.from);
      if (inAppRedirect) {
        navigate(inAppRedirect, { replace: true });
        return;
      }
      // Role-based redirect
      switch (role) {
        case "admin":
          ...
          break;
        case "teacher":
          navigate(ROUTES.teacherDashboard, { replace: true });
          break;
        default:
  ...
  }, [isAuthenticated, location.state, navigate, dispatch, role]);
